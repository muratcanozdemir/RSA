...
tic
if isempty(n),
   outfile=rsa_code_asym(filename,1);
else
   outfile=rsa_code_asym(filename,n,e);
end;
toc
tic
if isempty(n),
   rsa_code_asym(outfile,-1);
else
   rsa_code_asym(outfile,n,e,d);
end;
toc

[fp,fn,fe]=fileparts(filename);
dos(['fc /B ',filename,' c:\temp\tmptmp.decode']);
delete('c:\temp\tmptmp.decode');
delete(outfile);
return;
